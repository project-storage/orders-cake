...
export const fetchCakes = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("cakes/fetchCakes", async (_, { rejectWithValue }) => {
  try {
...
export const fetchCakeById = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("cakes/fetchCakeById", async (id, { rejectWithValue }) => {
  try {
...
export const createCake = createAsyncThunk<
  ...
  CakeData,
  { rejectValue: string }
>("cakes/createCake", async (cakeData, { rejectWithValue }) => {
  try {
...
export const updateCake = createAsyncThunk<
  CakeData,
  { id: string; data: Partial<CakeData> },
  { rejectValue: string }
>("cakes/updateCake", async ({ id, data }, { rejectWithValue }) => {
  try {
...
export const deleteCake = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("cakes/deleteCake", async (id, { rejectWithValue }) => {
  try {
...
const cakeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        deleteCake.fulfilled,
        (state, action: PayloadAction<void, string>) => {
          state.cakes = state.cakes.filter(
            (cake) => cake.id !== action.meta.arg
          );
          state.loading = false;
        }
      )
